...
function SocialIcon() {
  ...
  return (
    <div className='socialIcon'>
      ...
      <button onClick={() => setChangeLinkClicked(!changeLinkClicked)}>{changeLinkClicked ? "Hide textarea" : "Add Link"}</button>
      {
        changeLinkClicked && 
        <textarea value={linkVal} onChange={(e) => setlinkVal(e.target.value)} placeholder='enter link here'>
        </textarea>
      }

<div className="codeContainer">
        <p>{`<a class='socialIcon' href="${linkVal}"><img src="${icon.img}" alt="${icon.name}" /></a>`}</p>
        <button
          onClick={() =>
            navigator.clipboard.writeText(`<a class='socialIcon' href="${linkVal}"><img src="${icon.img}" alt="${icon.name}" /></a>`)
          }
        >
          Copy
        </button>
